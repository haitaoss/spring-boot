...
class SpringApplicationBannerPrinter {
    ...
    private Banner getBanner(Environment environment) {
        ...
        /**
         * 1. 读取属性 spring.banner.image.location 配置的文件(必须是图片吗？？)
         * 2. 读取 banner.[jpg | gif | png]
         * */
        banners.addIfNotNull(getImageBanner(environment));
        /**
         * 1. 读取属性 spring.banner.location 配置的文件(必须是文本吗？)
         * 2. 读取不到，默认找 banner.txt
         * */
    ...
    private Banner getTextBanner(Environment environment) {
        ...
        try {
            if (resource.exists() && !resource.getURL()
                    .toExternalForm()
